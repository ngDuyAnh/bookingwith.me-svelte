...
<div class="mt-4 flex">
    ...
    <div class="justify-start">
        <Button id="show-tooltip" on:click={() => handleEditCustomerBooking($customerBookingClickModal.customerBooking)} color="light" outline>
        ...
        {#if $business.businessInfo.scheduleManagement === BusinessScheduleManagement.NONE}
            <Button disabled={!ableToSendSmsReviewReminder || $customerBookingClickModal.customerBooking.smsReviewReminderSent} id="show-tooltip"
                    color="light" outline
    ...
    <div class="ml-auto justify-end content-center space-x-2">
        <!--None schedule management does not need to move the booking state unless to send review reminder-->
        {#if $business.businessInfo.scheduleManagement !== BusinessScheduleManagement.NONE}
            ...
            <Button class="bg-orange-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
                    on:click={handleLobbyClick}>Lobby</Button>
            <Button class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded"
                    on:click={handleServicingClick}>Servicing</Button>
            <Button class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded"
                    on:click={handleCompleteClick}>Complete</Button>
        {/if}
    </div>
</div>
