<script>
  ...
  import { findServiceBookingFromCustomerBooking } from "$lib/api/initialize_functions/utilitiy_functions/customer-booking-utility-functions.js";
  ...
  <div class="flex flex-col items-center space-y-1 mt-2 h-full">
    <div class="flex flex-row justify-center space-x-1 z-[2]">
      <Button
        ...
        on:click={() => (option = 0)}>Currently Servicing</Button
      >
      <Button
        color={option == 1 ? "blue" : "alternative"}
        on:click={() => (option = 1)}>Upcoming</Button
      >
