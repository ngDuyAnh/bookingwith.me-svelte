...
{#if $customerBookingEstimate.customerBooking.bookingState === CustomerBookingState.APPOINTMENT}
    {#if $business.businessInfo.scheduleManagement === BusinessScheduleManagement.NONE}
        <AppointmentNoneScheduleManagement/>
    {:else}
        <Appointment/>
    {/if}
{:else if $customerBookingEstimate.customerBooking.bookingState === CustomerBookingState.LOBBY}
    {#if $business.businessInfo.scheduleManagement === BusinessScheduleManagement.NONE}
        <LobbyNoneScheduleManagement/>
    {:else}
        <Lobby/>
    {/if}
{:else if $customerBookingEstimate.customerBooking.bookingState === CustomerBookingState.SERVICING}
    {#if $business.businessInfo.scheduleManagement === BusinessScheduleManagement.NONE}
        <ServicingNoneScheduleManagement/>
    {:else}
        <Servicing/>
    {/if}
{:else if $customerBookingEstimate.customerBooking.bookingState === CustomerBookingState.COMPLETED}
    <Completed/>
{/if}
