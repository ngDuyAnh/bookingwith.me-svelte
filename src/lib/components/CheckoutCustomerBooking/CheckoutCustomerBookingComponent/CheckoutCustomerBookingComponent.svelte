...
>
  <div bind:this={scrollContainer} class="scroll-container flex-grow space-y-1">
    <div
      class="cost-summary-wrapper flex justify-center w-full"
      style={cardStyle}
    >
      <div
        class="flex flex-row items-center space-x-2 bg-gray-100 p-3 rounded-b-none rounded-lg shadow w-full"
      >
        <UserCircleSolid class="h-6 w-6 text-blue-500" />
        <span class="font-medium text-gray-700"
          >{customerBooking.customer.customerName}</span
        >
        <span class="font-medium text-gray-700"
          >{formatPhoneNumber(customerBooking.customer.phoneNumber)}</span
        >
      </div>
    </div>
    <div class="w-full flex flex-col lg:flex-row flex-grow">
                  ...
                  <tr class={ind === 0 && index !== 0 ? "divide-y" : ""}>
                    ...
                    >
                      <div class="flex justify-between items-center pt-2">
                        ...
                        <span class="w-[100px] truncate"
                          >${individualServiceBooking.serviceCostAdjusted}</span
                        >
                    ...
                    >
                      <div class="flex justify-between items-center pt-2">
                        ...
                        <span class="w-32 text-center truncate"
                          >{individualServiceBooking.assignedEmployee
                            ? individualServiceBooking.assignedEmployee
                                .employeeName
                            : "Not Recorded"}</span
                        >
      ...
      <!-- Cost Summary -->
      <div
        class="cost-summary-wrapper w-full flex justify-center lg:flex-grow"
        style={cardStyle}
      >
        <Card class="w-full rounded-t-none" size="xl">
          ...
          <div class="flex justify-between mb-2 border-b-2">
            <div class="flex flex-row">
              ...
              />
              <Tooltip triggerdBy="#netSubTotal" class="z-10"
                >Net Subtotal = Subtotal - Discount</Tooltip
              >
              :
            </div>
          ...
          <div class="flex justify-between mb-2">
            ...
            <div
              class="flex justify-between bg-green-100 px-[3px] border-y-[2px] items-center mb-2"
            ...
            >
              ...
              <span
                ...
                class:!text-red-500={parseFloat(amountDue) !== 0}
                >${parseFloat(amountPaid).toFixed(2)}</span
              >
            </div>

            <div
              class="flex justify-between px-[3px] border-y-[2px] items-center mb-2"
              class:!bg-red-100={parseFloat(amountDue) !== 0}
            >
              ...
              <span
                class="text-lg font-medium text-green-500"
                class:!text-red-500={parseFloat(amountDue) !== 0}
                >${parseFloat(amountDue).toFixed(2)}</span
              >
            </div>
          </div></Card
        >
